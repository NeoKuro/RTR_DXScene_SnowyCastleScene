...
vertexOutputPacket main(vertexInputPacket IN) {
	...
	//Calculate Wave vertex normals tangents and bi tangents
	for (int i = 0; i < wave.Length; i++)
	{
		Po.y += evaluateWave(wave[i], Po.xz, Timer / TIMESCALE);
		float deriv = evaluateWaveDeriv(wave[i], Po.xz, Timer / TIMESCALE);
